...
class MedicalPathologyImport(models.TransientModel):
    ...
    def _icd10_process_diag(self, diag, category_record, parent_record=None):
        ...
        self.ensure_one()
        note = '\n'.join(self.__get_el_text_array(diag.find('inclusionTerm')))
    ...
    def __create_chapter_desc(self, chapter):
        use_additional = self.\
            __get_el_text_array(chapter.find('useAdditionalCode'))
        excludes_1 = self.__get_el_text_array(chapter.find('excludes1'))
        excludes_2 = self.__get_el_text_array(chapter.find('excludes2'))
        includes = self.__get_el_text_array(chapter.find('includes'))
        notes = self.__get_el_text_array(chapter.find('notes'))
        if use_additional:
